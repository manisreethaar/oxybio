...
def refine_hero():
    try:
        ...
        # We'll replace the entire <section class="structure-section page-hero"... to <!-- INNOVATIVE KNOWLEDGE PARTNER BANNER -->
        import re
        pattern = re.compile(
            r'<section class="structure-section page-hero".*?(?=<!-- INNOVATIVE KNOWLEDGE PARTNER BANNER -->)',
            re.DOTALL
        )
        ...
        <section class="structure-section page-hero"
            ...
            <div class="container hero-grid-layout" style="position:relative; z-index:2; display: grid; grid-template-columns: 1.2fr 0.8fr; gap: 4rem; align-items: center;">
                ...
                <!-- RIGHTSIDE ANIMATION -->
                <div class="hero-animation-wrapper reveal" style="transition-delay: 0.3s;">
        ...
        if pattern.search(idx):
            ...
            with open(r'e:\OXYBIO\index.html', 'w', encoding='utf-8') as f:
                f.write(idx)
            print("Successfully refined Hero layout!")
        else:
