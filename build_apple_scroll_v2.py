...
# Target exactly the Solution section by identifying the unique text "So we built one. Meet Oxygen."
# It ends at the closing </section> before "SCIENCE & PILLARS SECTION"
match = re.search(r'<!--[^>]*?SOLUTION / PRODUCTS SECTION[^>]*?-->.+?<section class="structure-section"[^>]*>.+?</section>', html, re.DOTALL)

if match:
    section_html = match.group(0)
    
    # Extract the VITALITY, CLARITY, MOMENTUM blocks to reuse them accurately.
    vitality_match = re.search(r'<!-- VITALITY -->(.*?)<!-- CLARITY -->', section_html, re.DOTALL)
    clarity_match = re.search(r'<!-- CLARITY -->(.*?)<!-- MOMENTUM -->', section_html, re.DOTALL)
    momentum_match = re.search(r'<!-- MOMENTUM -->(.*?)</section>', section_html, re.DOTALL)
