...
def rewrite_hero_exact():
    try:
        with open(r'e:\OXYBIO\index.html', 'r', encoding='utf-8') as f:
            idx = f.read()

        start_marker = '<div class="container hero-grid-layout" style="position:relative; z-index:2;">'
        ...
        if start_idx != -1 and end_idx != -1:
            ...
            /* Make the parent section flex centered */
            .hero-flex-centered {
            ...
            <script>
                document.addEventListener("DOMContentLoaded", () => {
                    const canvasMesh = document.getElementById('biology-mesh');
                    if(canvasMesh && canvasMesh.parentElement) {
                        canvasMesh.parentElement.classList.add('hero-flex-centered');
                    }
                });
            </script>

            <!-- BACKGROUND ANIMATION (Subtle mathematical backdrop) -->
            <div class="hero-animation-wrapper reveal" style="position:absolute; top:50%; left:50%; transform:translate(-50%, -50%) scale(1.6); opacity: 0.06; z-index: 1; pointer-events: none; transition-delay: 0.2s;">
            ...
            <div class="container" style="position:relative; z-index:2; display:flex; flex-direction:column; align-items:center;">
                ...
                <div class="flow-left reveal" style="width: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center;">

                    <!-- Tags -->
                    <div style="display:flex; justify-content:center; align-items:center; flex-wrap:wrap; gap:10px; margin-bottom: 2.5rem;">
                    ...
                    <h2 class="v2-split-text"
                        style="font-family: var(--font-serif); font-size: clamp(1.8rem, 4vw, 2.75rem); line-height: 1.2; font-weight: 700; color: var(--text-main); margin-top: 1.5rem; opacity: 0; letter-spacing: -0.02em;">
                        Advanced Functional Foods.<br>Powered by Fermentation.
                    </h2>

                    <p class="subtext editorial-col"
                        style="margin-top:var(--space-md); font-size:var(--text-xl); line-height:var(--leading-relaxed); max-width: 800px; margin-left: auto; margin-right: auto; text-align: center;">
        ...
        <!-- INNOVATIVE KNOWLEDGE PARTNER BANNER -->"""
            ...
            new_idx = idx[:start_idx] + new_container + idx[end_idx + len(end_marker):]
            
            with open(r'e:\OXYBIO\index.html', 'w', encoding='utf-8') as f:
                f.write(new_idx)
            print("Successfully rewrote Hero exact layout!")
        else:
